...
class LightroomCatalogIngestionProgressTracker:
    ...
    def __init__(
        ...
        *,
        ...
        self._state = LightroomCatalogIngestionProgressState()
    ...
    @property
    def counters(self) -> LightroomCatalogIngestionProgressState:
        return self._state

    def start_phase(self, *, phase: str, total: int | None) -> None:
        logger.info(
            "lightroom catalog ingest phase started",
    ...
    def finish_phase(self) -> None:
        logger.info(
            "lightroom catalog ingest phase completed",
            extra={
    ...
    def finish_run(self, *, status: str) -> JobProgressSnapshot:
        ...
        logger.info(
            "lightroom catalog ingest completed",
            extra={
                "run_id": self._engine.state.run_id,
                "status": status,
    ...
    def fail_run(self) -> JobProgressSnapshot:
        ...
        logger.error(
            "lightroom catalog ingest failed",
            extra={
                "run_id": self._engine.state.run_id,
                "phase": self._engine.state.phase,
                **self._progress_payload(),
    ...
    def _log_heartbeat_if_written(self, snapshot: JobProgressSnapshot) -> None:
        ...
        logger.info(
            "lightroom catalog ingest heartbeat written",
            extra={
                "run_id": self._engine.state.run_id,
                "phase": self._engine.state.phase,
                "last_heartbeat_at": (
